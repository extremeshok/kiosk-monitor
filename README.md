...
- **Dual-display** support — tested end-to-end on **both Wayland (labwc) and X11** (Chromium on one HDMI + VLC RTSP on the other, both kiosk-fullscreen, both supervised independently). On Wayland, **fullscreen** windows are routed to the right HDMI by *placement*: the managed `rc.xml` sets `<placement><policy>cursor</policy></placement>` and the watchdog warps the pointer onto each instance's output (via `wlrctl`) immediately before launching it, so the surface maps fullscreen there. This is necessary because labwc's `MoveToOutput` action is a **no-op on fullscreen windows** — so the window rule alone (Chromium matched by `identifier=`, VLC by `title=`) cannot move a fullscreen VLC; the rules are kept for `serverDecoration`/`skipTaskbar` and as a backup for non-fullscreen windows. On X11 each instance is routed to its configured output after launch **and on every health-check tick** via `wmctrl` (drop fullscreen → move/resize to target rect → re-add fullscreen), which also catches VLC's `--loop` behaviour of destroying and recreating its video window on every RTSP reconnect. On Wayland a post-launch *settle* plus a `WAYLAND_REROUTE_GRACE`-second window of re-nudges backs this up for late-mapping surfaces (an **HEVC** camera can't create its window until the first keyframe carrying VPS/SPS/PPS arrives, seconds after launch).
...
- **`CHROME_VIA_JSMPEG` for the Frigate Birdseye chrome freeze** — kiosk-monitor's bundled Chromium extension drops a `world: "MAIN"` content script that intercepts the page's `/api/config` fetch + XHR responses and rewrites `birdseye.restream` to `false` before Frigate's React WebUI reads it. Frigate's `LiveBirdseyeView` then takes its native JSMpeg branch, mounting a canvas player that consumes `/live/jsmpeg/birdseye` directly — never touches MSE, never hits the chunk-demuxer freeze ([issue #1](https://github.com/extremeshok/kiosk-monitor/issues/1) root cause: Chromium 147's `ChunkDemuxer` cycling on `DEMUXER_UNDERFLOW` against the fmp4 stream Frigate's go2rtc emits when `birdseye.restream: true`; reproducible on hardware as different as Pi 5 V3D and Haswell i965). Server-side config is untouched — `rtsp://host:8554/birdseye` keeps working for VLC/HomeKit in parallel. The knob is a tristate: `auto` (default — probe Frigate at startup, enable the shim only when `restream:true` is observed), `true` (always inject the shim), `false` (never inject). `--doctor` reports the resolved state. Operator keeps the full Frigate WebUI (camera labels, sidebar, click-to-zoom).
- **Doctor warning for chrome + Frigate Birdseye** — `--doctor` fires whenever a chrome instance points at a Birdseye URL, regardless of hardware. The warning lays out the two viable fixes (`birdseye.restream: false` on Frigate, or `CHROME_VIA_JSMPEG=true` on kiosk-monitor) and explains the trade-offs. When `CHROME_VIA_JSMPEG=true` is already set on a matching instance, the doctor reports OK instead of warning.
...
- **VLC live-stream hardening (`VLC_RTSP_HARDENING`)** — for `rtsp`/`rtsps`/`rtmp`/`rtmps`/`udp` URLs the VLC launcher pins a jitter buffer, forces RTSP-over-TCP, and sets `--clock-jitter=0`. This stops the "grey screen, then footage returns" failure where an uneven restream cadence (e.g. Frigate birdseye via go2rtc, which emits few/no frames while idle) makes VLC inflate `pts_delay` until it loses the reference clock and stops painting. Pair with Frigate `birdseye.idle_heartbeat_fps` (a small non-zero value) to remove the cadence gap at the source. All flags overridable via `VLC_EXTRA_ARGS`.
- **Debounced output-pause (`OUTPUT_MISS_GRACE`)** — an instance is only paused (and its window torn down) after its display reads absent on N consecutive watchdog polls, and the pause check falls back to the kernel DRM connector status (`/sys/class/drm`). A single transient `wlr-randr` miss can no longer kill a feed to the desktop.
...
- `VLC_NETWORK_CACHING` — ms of network caching (raise for flaky RTSP). When set, always wins over the hardening default below.
- `VLC_RTSP_HARDENING` — `auto` (default) / `true` / `false`. With `auto`, live-stream URLs (`rtsp`/`rtsps`/`rtmp`/`rtmps`/`udp`) get a jitter buffer, `--rtsp-tcp` (rtsp/rtsps only), and `--clock-jitter=0`. This stops the **"grey screen, then footage returns"** failure where an uneven restream cadence (e.g. Frigate birdseye via go2rtc) makes VLC inflate `pts_delay` until it loses the reference clock and stops painting. See *Runtime behaviour*.
- `VLC_CLOCK_JITTER` — VLC `--clock-jitter` value the hardening applies (default `0` — disables the `pts_delay` runaway).
- `VLC_CLOCK_SYNCHRO` — VLC `--clock-synchro` value the hardening applies. `0` (default) makes VLC free-run on its own clock instead of resetting to the stream's RTCP reference clock (the reset is what flashes the screen grey on a go2rtc/birdseye feed); `-1` restores VLC's default; `1` forces synchronization on. `0` trades a little latency for no clock-reset freezes.
- `VLC_NETWORK_CACHING_DEFAULT` — network cache (ms) the hardening applies for live streams when `VLC_NETWORK_CACHING` is unset (default `1500`).
- `VLC_EXTRA_ARGS` — free-form args appended **last** to the VLC command line, so they override the hardening defaults (e.g. `--no-rtsp-tcp`, a different `--clock-jitter=N`).
...
| `VLC_STALL_RETRIES`     | Identical-frame hashes before restart (VLC)                     | `20` |
| `OUTPUT_MISS_GRACE`     | Consecutive missed display polls before pausing an instance     | `3` |
| `WAYLAND_REROUTE_GRACE` | Seconds after each (re)launch the watchdog re-asserts labwc routing each tick (Wayland multi-output; `0` disables) | `60` |
| `WAYLAND_REROUTE_SETTLE_TICKS` | Extra labwc re-route nudges in the post-launch settle | `3` |
| `WAYLAND_REROUTE_SETTLE_INTERVAL` | Seconds between settle re-nudges | `4` |
| `WAYLAND_CURSOR_ROUTING` | Place fullscreen windows on their output by warping the pointer there before launch (`auto`/`true`/`false`; needs `wlrctl`) | `auto` |
...
- VLC runs with `--intf=dummy` (no UI), `--fullscreen`, and a unique `--logfile` as a process fingerprint. For live-stream URLs the launcher also appends the `VLC_RTSP_HARDENING` flags (`--clock-jitter`, `--rtsp-tcp`, a default `--network-caching`) ahead of `VLC_NETWORK_CACHING`/`VLC_EXTRA_ARGS` so operator overrides win.
- **Grey-screen / clock-loss on restreamed feeds:** a restreamed Frigate birdseye (go2rtc re-encode) delivers frames with an uneven cadence — during idle stretches it emits few/no frames, so VLC's PCR arrives "late", it grows `pts_delay` (default `--clock-jitter` window is 5000ms), and once it gives up it loses the reference clock and stops painting, showing a solid grey until frames resume. The tell in `/tmp/kiosk-monitor-vlc-<id>.log` is repeated `ES_OUT_SET_(GROUP_)PCR is called too late (pts_delay increased to …)`, `buffer deadlock prevented`, and `no reference clock`. `VLC_RTSP_HARDENING` (on by default for live URLs) caps this with `--clock-jitter=0` + `--rtsp-tcp` + a jitter buffer; the complete fix also sets Frigate `birdseye.idle_heartbeat_fps` to a small non-zero value so the stream never goes silent.
- Window placement is computed from `wlr-randr --json` on Wayland or `xrandr --query` on X11: each instance launches at its output's top-left with that output's native resolution, and the compositor fullscreens it to the containing monitor. The output query is non-destructive — a transient `wlr-randr`/`xrandr` failure keeps the last-known-good output list rather than reading as "all displays gone" — and an instance is only paused after its display is missing for `OUTPUT_MISS_GRACE` consecutive polls, with a `/sys/class/drm` connector-status fallback as kernel ground truth.
...
- v6.8.7 added a conditional `UseChromeOSDirectVideoDecoder` disable on legacy i965 VAAPI hardware, hypothesising the Frigate Birdseye chrome freeze was a VAAPI quirk. v6.11's Chromium DevTools Protocol Media-domain re-diagnosis ruled that out — the freeze is Chromium 147's `ChunkDemuxer` cycling on `DEMUXER_UNDERFLOW` against the fmp4 stream Frigate's go2rtc emits when `birdseye.restream: true`, reproducible on Pi 5 V3D (no libva at all) as well as on i965 boxes. Forcing `UseChromeOSDirectVideoDecoder=disabled` actually made the freeze faster on Pi 5, so v6.11 removes the conditional entirely. The real fix lives at the MSE-vs-JSMpeg level: see `CHROME_VIA_JSMPEG` above, or set `birdseye.restream: false` on the Frigate side.
