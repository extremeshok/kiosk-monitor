...
- **Hardware video decode preserved** when the distro chromium ships VAAPI flags in `/etc/chromium.d/*`: kiosk-monitor merges its own `--enable-features=` / `--disable-features=` with whatever is already there before launch, so software decode never silently kicks in (which used to freeze H.264 streams like Frigate Birdseye on modest CPUs after a minute or two).
...
- Chromium launches with merged `--enable-features=` / `--disable-features=` switches: kiosk-monitor parses `/etc/chromium.d/*.conf`, `/etc/chromium-browser/default`, `/etc/chromium*/customizations/*`, and `~/.config/chromium-flags.conf` for any feature lists already configured by the distro and combines them with its own additions before passing one switch each. This is what keeps `VaapiVideoDecoder` and friends enabled on Debian/Ubuntu/Mint chromium — passing a bare `--enable-features=…` of our own would otherwise be last-wins and silently disable hardware video decode. The kiosk-mode triplet (`--disable-background-timer-throttling`, `--disable-renderer-backgrounding`, `--disable-backgrounding-occluded-windows`) is also passed unconditionally to keep video players running while the compositor decides whether the kiosk window is occluded.
