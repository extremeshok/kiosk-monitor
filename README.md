...
- **Frigate "Preferred Live Mode" pre-seeding** — when Chromium's MSE pipeline is unstable on the hardware (issue #1: Birdseye stalls after a few minutes on Haswell + i965 while Firefox runs the same MSE stream cleanly), kiosk-monitor can pre-seed Frigate's live-mode preference via localStorage from the helper extension at `document_start`. `FRIGATE_LIVE_MODE=jsmpeg` forces the canvas-over-WebSocket player and bypasses MSE entirely; `FRIGATE_LIVE_MODE=auto` (the default) smart-defaults to `jsmpeg` only on legacy-i965 boxes and leaves Frigate's own default alone everywhere else. The seed is written under several alias keys (`frigate-live-mode`, `live-mode`, `preferred-live-mode`, etc.) so it survives Frigate version drift.
...
- `FRIGATE_LIVE_MODE` — pre-seed Frigate's "Preferred Live Mode" via localStorage. `""` leaves Frigate's own default alone. `"auto"` smart-defaults to `jsmpeg` on hardware where Chromium MSE is known to be unstable (legacy i965 VAAPI driver — Sandy Bridge through Coffee Lake Intel iGPUs, see [issue #1](https://github.com/extremeshok/kiosk-monitor/issues/1)) and leaves it alone elsewhere. `"off"` / `"none"` opt out explicitly. `"jsmpeg"`, `"mse"`, `"webrtc"` pin a specific player. Useful when Birdseye freezes after a few minutes on Chromium while running cleanly in Firefox — that's the Chromium-MSE-on-i965 footgun.
- `FRIGATE_LIVE_STORAGE_KEY` — override the primary localStorage key the seed is written to (default: `frigate-live-mode`). The helper writes the same value to a list of known alias keys so the seed survives Frigate version drift; this knob is for forks or future Frigate versions that rename the canonical key.
